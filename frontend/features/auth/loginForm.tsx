  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isSignUp) {
        ...
        if (!authData.user)
          throw new Error('Registration failed. Identity record was not created.');
      ...
      if (authError) throw authError;
      if (!authData.session) throw new Error('Failed to establish a valid user session.');
      if (!API_URL)
        throw new Error('Frontend API configuration is missing. Check NEXT_PUBLIC_API_URL.');
      ...
      if (!response.ok) throw new Error(result.error || 'Profile verification failed.');

      router.replace('/dashboard');
    } catch (err: unknown) {
      if (err instanceof Error) setErrorMsg(err.message);
      else setErrorMsg('An unexpected authentication error occurred.');
    } finally {
      setLoading(false);
    }
  };
